...
%% Title information
\title{\vspace{-1cm}Project nº 1: Loan Approval Prediction with Linear and Bayesian Models\\
    Advanced Machine Learning (MDS)}

\author{Martin Tazón \and Daniel Wero\'nski \and Martin de Peretti}
...
We address a supervised binary classification problem: predicting loan approval from mixed tabular data containing numerical features. Our objective is to evaluate three foundational methods: a discriminative Generalized Linear Model (Logistic Regression), a Bayesian generative classifier (Naive Bayes), and a Support Vector Machine method (Linear and RBF). \\
...
The dataset used in this work includes mostly numerical features, with only one ordinal feature describing applicants’ profiles and their corresponding loan approval outcomes.
...
The task of predicting loan approval is a widely studied binary classification problem~\cite{sheikh2020approach, arun2016loan, sinap2024comparative}. A common and well-established approach in the literature is to perform a comparative analysis of various machine learning algorithms to identify the most effective and interpretable model. These studies frequently compare foundational classifiers, such as the ones used in this project, against more complex ensemble methods like Random Forest.

Many studies have specifically focused on comparing Logistic Regression and Support Vector Machines for this task, given their robustness in binary classification. In these comparisons, Logistic Regression is often highlighted for its simplicity and high interpretability, which is particularly valuable for financial decision-making where understanding the influence of features (like income or credit history) is crucial. Other works have included Naive Bayes in the comparison, noting that this trio of algorithms (NB, LR, and SVM) represents reliable, popular, and foundational supervised learning methods for classification.\\

Our work connects to the core theoretical comparison between generative and discriminative models. Naive Bayes is a classic generative model, as it learns the joint probability distribution $P(\mathbf{x}, y)$ by modeling the likelihood $P(\mathbf{x}|y)$ and the prior $P(y)$. In contrast, Logistic Regression and SVMs are discriminative models, as they directly learn the conditional probability $P(y|\mathbf{x})$ or the decision boundary between classes, respectively. This distinction is central to financial tasks; discriminative models are often preferred for credit scoring and fraud detection due to their typically higher predictive accuracy.
...
The dataset, titled \textit{Loan Approval Dataset}~\cite{devedward2024loan}, was obtained from Kaggle and contains information about applicants and their corresponding loan approval status. Upon inspcting the details of the data we found it to be artificially generated using Faker Library.
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lcccc@{}}
        ...
        RBF-Kernel SVM & $\mathbf{0.94}$ & $\mathbf{0.94}$ & $\mathbf{0.99}$ & $4.2$ \\
        \bottomrule
    \end{tabular}
\end{table}
